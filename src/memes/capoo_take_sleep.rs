...
// 主要的表情包生成函数
fn capoo_take_sleep(images: Vec<InputImage>, _: Vec<String>, _: NoOptions) -> Result<Vec<u8>, Error> {
    ...
    for i in 0..28 {
        ...
        // 加载预定义的咖波贴图帧
        let frame = load_image(format!("capoo_take_sleep/{i}.png"))?;
        ...
        // 调整输入图像到指定尺寸
        let image = image.resize_exact((w, h)).rotate(-35.0);
