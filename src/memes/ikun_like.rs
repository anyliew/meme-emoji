...
const DEFAULT_TEXT: &str = "坤坤喜欢这个";

fn ikun_like(images: Vec<InputImage>, texts: Vec<String>, _: NoOptions) -> Result<Vec<u8>, Error> {
    let text = if !texts.is_empty() { &texts[0] } else { DEFAULT_TEXT };

    let frame = load_image("ikun_like/0.png")?;
    let mut surface = frame.to_surface();
    let canvas = surface.canvas();
    canvas.draw_text_area_auto_font_size(
        IRect::from_ltrb(5, frame.height() - 60, frame.width() - 5, frame.height() - 10),
        text,
        20.0,
        40.0,
        text_params!(
