...
// 主要的表情包生成函数
fn kurogames_rover_lick(images: Vec<InputImage>, _: Vec<String>, _: NoOptions) -> Result<Vec<u8>, Error> {
    ...
    for i in 0..13 {
        ...
        // 加载预定义的咖波贴图帧
        let frame = load_image(format!("kurogames_rover_lick/{i}.png"))?;
        ...
        // 调整输入图像到指定尺寸
        let image = image.resize_exact((w, h)).rotate(-15.0);
