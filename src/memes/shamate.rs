...
// 主要的表情包生成函数
fn shamate(images: Vec<InputImage>, _: Vec<String>, _: NoOptions) -> Result<Vec<u8>, Error> {
    ...
    // 获取输入的第一张图像并转换为正方形
    let image = images[0].image.square().circle();
    ...
    for i in 0..35 {
        ...
        // 加载预定义的咖波贴图帧
        let frame = load_image(format!("shamate/{i}.png"))?;
